...
readTimeout :: Device -> Int -> Int -> IO (Maybe ByteString)
readTimeout dev n timeout = allocaBytes n $ \b -> do 
  n' <- hid_read_timeout dev b (fromIntegral n) (fromIntegral timeout)
  checkWithHidError (n' /= -1) dev "Read failed" "hid_read_timeout returned -1"
  if (n' == 0) 
    then pure Nothing 
    else Just <$> packCStringLen ( b, fromIntegral n' )
